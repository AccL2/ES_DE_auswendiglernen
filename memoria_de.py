...
# ── CARGA: frases principales (ttl generoso, cambia poco) ──
@st.cache_data(ttl=60)
def cargar_frases(_bust):
    url = f"{SHEET_BASE_URL}&nocache={_bust}"
...
# ── CARGA: hoja Progreso (ttl=1 + bust aleatorio → siempre fresca) ──
@st.cache_data(ttl=1)
def cargar_progreso(_bust):
    url = f"{SHEET_BASE_URL}&sheet=Progreso&nocache={_bust}"
...
def obtener_contador_diario():
    try:
        ...
        df = cargar_progreso(_bust=random.randint(1, 999999)).copy()
...
# ── CARGA INICIAL ──
try:
    df_total = cargar_frases(_bust=random.randint(1, 999999))
...
indice_fila_google_sheet = int(
    df_isla_completa.index[df_isla_completa['Castellano'] == castellano_texto].tolist()[0]
) + 2
