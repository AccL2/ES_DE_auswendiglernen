...
def obtener_contador_diario():
    try:
        hoy_guiones = datetime.now().strftime("%Y-%m-%d")
        hoy_barras = datetime.now().strftime("%d/%m/%Y")
        
        if df_progreso.empty:
            return 0
            
        df_prog_copy = df_progreso.copy()
        col_fecha = [c for c in df_prog_copy.columns if 'fecha' in c.lower()]
        
        if not col_fecha:
            return 0
        
        nombre_col = col_fecha[0]
        df_prog_copy[nombre_col] = df_prog_copy[nombre_col].astype(str).str.strip().str.split().str[0]
        
        fila_hoy = df_prog_copy[
            (df_prog_copy[nombre_col] == hoy_guiones) | 
            (df_prog_copy[nombre_col] == hoy_barras)
        ]
        
        if not fila_hoy.empty and pd.notna(fila_hoy.iloc[0]['Cantidad']):
